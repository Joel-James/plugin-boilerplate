...
spl_autoload_register( 'ddb_auto_loader' );

/**
 * Dynamically loads the class attempting to be instantiated elsewhere in the
 * plugin by looking at the $class_name parameter being passed as an argument.
 *
 * The argument should be in the form: DD_Boilerplate\Namespace. The
 ...
 * @since 1.0.0
 */
function ddb_auto_loader( $class_name ) {

	// If the specified $class_name does not include our namespace, duck out.
	if ( false === strpos( $class_name, 'DD_Boilerplate' ) ) {
		return;
	}
	...
	if ( file_exists( $filepath ) ) {
		...
		wp_die( printf( __( 'The file attempting to be loaded at %s does not exist.', 'dd-boilerplate' ), $filepath ) );
	}
}